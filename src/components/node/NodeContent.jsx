...
const NodeContent = ({ 
  ...
  const getNodeStyle = () => {
    const focusClasses = isFocused 
      ? '-m-4 ring-2 ring-blue-500 ring-offset-2 scale-[1.02]' 
    ...
    <div onClick={handleNodeClick} className="p-4">
      {isEditing ? (
        <div className={`space-y-2 p-4 bg-white rounded-lg m-1 transition-all duration-200 ${
          isFocused ? '-m-4 ring-2 ring-blue-500 ring-offset-2' : 'hover:ring-1 hover:ring-blue-300 hover:ring-offset-1'
        }`}>
          <Input
            value={localTitle}
            onChange={(e) => setLocalTitle(e.target.value)}
