...
const NodeContent = ({ style, isEditing, node, localTitle, localDescription, handleBlur, setLocalTitle, setLocalDescription, handleNodeClick }) => {
  const getNodeStyle = () => {
    switch (style) {
      case 'compact':
        return (
          <div className={`w-10 h-10 p-1 bg-white rounded-full ring-offset-background transition-all ${isEditing ? 'ring-2 ring-ring ring-offset-2' : ''}`}>
            <img src="/default-image.png" alt="" className="w-8 h-8 rounded-full" />
          </div>
        );
      case 'expanded':
        return (
          <div className={`flex items-center p-2 bg-white rounded-lg ring-offset-background transition-all ${isEditing ? 'ring-2 ring-ring ring-offset-2' : ''}`}>
      ...
      case 'postit':
        return (
          <div className={`w-[240px] h-[200px] p-4 bg-yellow-100 rounded-sm shadow-md ring-offset-background transition-all ${isEditing ? 'ring-2 ring-ring ring-offset-2' : ''}`}>
            {isEditing ? (
  ...
  return (
    <div onClick={handleNodeClick} className="p-4">
      {isEditing ? (
        <div className="space-y-2 p-4 bg-white rounded-lg ring-2 ring-ring ring-offset-2">
          <Input
            value={localTitle}
            onChange={(e) => setLocalTitle(e.target.value)}
            onBlur={handleBlur}
